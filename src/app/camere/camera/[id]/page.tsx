...
import Popup from "@/components/ClientGallery";
...
export default async function SingleRoomPage({ params }: PageProps) {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/getAll?id=${id}`);
    ...
    const multipleRooms = await fetch(`http://localhost:3000/api/getAll?id=`);
